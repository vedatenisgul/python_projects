...
def save_to_the_file():
    ...
    if len(website) == 0 or len(email_or_user_name) == 0 or len(password) == 0:
        ...
        continue_process = messagebox.askokcancel(title=website, message=f"These are details entered: "
                                                      f"\nEmail/Username: {email_or_user_name}"
                                                      f"\nPassword: {password}"
                                                      f"\nIs it ok to save?")
        if continue_process:
            with open("data.txt", "a") as file:
                file.write(f"{website} | {email_or_user_name} | {password}\n")
            reset_entries()
